...
// )}
// ============================================================
// RESPONSIVE MOBILE - CSS PUR + MENU MOBILE AUTOMATIQUE
// ============================================================
...
// Fonction pour créer le menu mobile (s'exécute après le chargement de React)
setTimeout(() => {
  // Vérifier si le menu mobile existe déjà
  if (document.querySelector('.mobile-header')) return;
  
  // Trouver le main element
  const mainElement = document.querySelector('main');
  if (!mainElement) return;
  
  // Récupérer les éléments de navigation depuis la sidebar
  ...
  sidebar.querySelectorAll('nav > div').forEach(el => {
    ...
    const hasBadge = !!el.querySelector('span:last-child[style*="marginLeft: auto"]');
    if (label) {
      navItems.push({ icon, label, hasBadge, element: el });
    }
  });
  ...
  drawer.innerHTML = `
    <div class="drawer-header">
      <div class="logo-sub">Gym Management</div>
      <div class="logo-main">NOUVEL <span class="logo-accent">ÉLAN</span></div>
      <div style="font-size:9px;color:#333;margin-top:4px;">v2.1.0</div>
    </div>
  ...
  navItems.forEach((item, idx) => {
    ...
    `;
    navDiv.onclick = () => {
      // Simuler un clic sur l'élément de la sidebar
      if (item.element) {
        item.element.click();
      }
  ...
  const footerSidebar = sidebar.querySelector('div:last-child');
  if (footerSidebar) {
    const drawerFooter = drawer.querySelector('.drawer-footer');
    drawerFooter.innerHTML = footerSidebar.innerHTML;
    // Réattacher les événements du footer
    drawerFooter.querySelectorAll('button').forEach(btn => {
      const originalBtn = footerSidebar.querySelector(`button:has-text("${btn.innerText}")`) || 
                          Array.from(footerSidebar.querySelectorAll('button')).find(b => b.innerText === btn.innerText);
  ...
  // Mettre à jour l'élément actif lors du changement de vue
  const observer = new MutationObserver(() => {
    const activeSidebarItem = sidebar.querySelector('div[style*="background: #1a1a1a"], div[style*="color: #4ade80"]');
    if (activeSidebarItem) {
      const activeLabel = activeSidebarItem.childNodes[1]?.textContent?.trim();
      drawer.querySelectorAll('.drawer-nav-item').forEach(item => {
        if (item.innerText.includes(activeLabel || '')) {
